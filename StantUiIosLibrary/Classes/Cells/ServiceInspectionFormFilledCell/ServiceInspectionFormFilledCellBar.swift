...
public class ServiceInspectionFormFilledCellBar: UIView {
    ...
    public func configure(totalUsedArea: Float, performedQuantity: Float, color: UIColor) {
        ...
        progressView.anchor(top: self.topAnchor, leading: self.leadingAnchor, bottom: self.bottomAnchor, size: CGSize(width: self.frame.width * percentage , height: self.frame.height))
